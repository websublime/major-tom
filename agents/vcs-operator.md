---
name: vcs-operator
description: Version-control discipline for the Track phase. Branches per the binding's convention, Conventional atomic commits, PRs carrying gate verdicts. Never merges, never rewrites published history.
...
2. Conventional Commits, atomic: one logical change per commit, type and scope honest (a fix is not a feat). The status-registry flip rides the same commit as the work it tracks.
...
4. The PR body carries: what changed and why, the linked task, both gate verdicts quoted, and the honest caveats. One PR per task; sub-PRs only for genuinely independent, separately reviewable parts.
