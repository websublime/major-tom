...
```
/think <task>       full loop on the task (default)
/think plan <task>  Steps 0-3 only: classify, define done, gather evidence, deliver the plan, stop
/think audit        grade the work already done in this conversation against the loop (see Modes)
/think report       rewrite the answer you were about to send per Step 6
```
...
**plan** - run Steps 0 to 3 and stop. Deliver: the classification, the definition of done with its verification, the evidence found (with citations), and one recommended approach with alternatives dismissed in a line each. Do not touch any file, with one exception: if the binding names a landing path for a plan that will feed a design contract, write it there and say you did. Nothing else in this mode writes.
