...
4. **Hunt the classic frauds**, in order of real-world frequency:
   ...
   The full catalogue is `think`'s `references/failure-modes.md`; use it as the checklist when the work is large.
   **Non-code work is judged by its domain's fraud table.** If the work is marketing/content, research, data analysis, business/ops, a design contract (a spec, a decision record, an SSOT patch: `design-contracts.md`, whose six frauds include absolute negatives asserted without a search, scope dropped on restatement, and costs tilted toward the recommendation), or another covered sector, read the matching adapter in `think`'s `references/domains/` and hunt ITS fraud table (fabricated statistics, stale figures, budget fiction, silent data cleaning...) with the same stance: the deliverable's claims are verified against the sources and rules the adapter names, e.g. copy checked line-by-line against `brand.md`, figures re-fetched, arithmetic recomputed.
...
`/prove suite <target>` runs the think trap suite against a target configuration: a newly installed skill, a different model, a modified prompt. It needs the repo's `eval/` directory. If this skill was installed as the plugin, `eval/` is already in the plugin's install directory (the plugin source is the repo itself); locate it relative to this SKILL.md (`../../eval/`). Only standalone-skill installs need a separate clone of `https://github.com/websublime/major-tom`.
