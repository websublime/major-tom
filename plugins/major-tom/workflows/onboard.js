  ...
  () =>
    agent(
      [
        ...
        `Then generate the dashboard (D31, snapshot schema v2 per D33). Build the snapshot by running, from the target repo root: node ${pre2.pluginRoot}/app/snapshot.js --repo . --out <snapshot.json>`,
        'where <snapshot.json> is a path inside a temporary directory outside the target repository: the snapshot is an intermediate, never a file the repository keeps.',
        'The script owns the whole snapshot computation and is the single implementation of the D33, D41 and D42 window rules: it reads .claude/major-tom.json, walks the persistence root, runs git, and writes every key of schema v2 (generatedAt, config, git, knowledge, decisions, timeline, with lastRun and roadmap omitted). Never recompute, complete or post-process any part of what it writes, and never assemble a snapshot by hand. If it exits non-zero, report that as a failure; never hand-edit around it.',
        'Then run:',
        ...
        'You compute nothing in this step: you run the two scripts and report what they did.',
