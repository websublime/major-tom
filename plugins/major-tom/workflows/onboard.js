  ...
  () =>
    agent(
      [
        ...
        '- git: the commits inside the same window as the timeline key below, so both sides of the merged reading end at the same point (D41): the commits of the last 30 days, extended to the 50 most recent when the last 30 days yield fewer, and cut to at most 500. Each entry is {hash, date (ISO), author, subject, kind, add, del}. kind is the conventional-commit prefix of the subject (feat, fix, docs, test, chore, refactor) or "other"; add/del come from git log --numstat totals per commit. Empty array if no commits.',
        '- knowledge: {files: [{path, type, size, updated, frontmatter, body, truncated}]} for the OKF concepts under the persistence root, and for nothing else (D28): a concept is every non-reserved .md file carrying YAML frontmatter. Exclude the reserved index.md, which is the bundle index and not a concept, and exclude every non-.md file; today those are runs/intents.log (the D36 trivial and task intent lines, which D41 routes into the timeline key once that key is built) and the generated dashboard.html (the inject step below writes it inside the persistence root, so collecting it would embed the previous dashboard into its successor on every re-onboard). path is relative to the persistence root; frontmatter is the parsed YAML frontmatter object; type comes from frontmatter.type; updated is the file mtime as ISO. Window caps (D33): body is included only up to 32 KB per file (set truncated: true and cut cleanly when over) and only while the running total of embedded bodies is under 1 MB, walking files in index.md order first (read index.md for that ordering only, it is never itself an entry), then the rest; beyond the caps omit body entirely.',
        ...
        '- timeline: {events: [{at, kind, tier, type, promptId, sessionId, summary, summaryTruncated, path}], window: {days: 30, floorEvents: 50, ceilingEvents: 500, byteBudget: 262144}, omitted: {count, oldestKept, reason}}, the merged event history the dashboard reads (D41). window carries those four literal numbers always, whether or not any limit bit. Per event: at is ISO 8601; kind is "intent" or "run"; tier is trivial, task or substantive for intents and null for runs; type is the intent request type and null for runs; promptId and sessionId are strings or null; summary is at most 200 characters, with summaryTruncated true when it was cut; path is relative to the persistence root when the event has a file and null for a log line. events is sorted newest first.',
        '  Sources, all under the persistence root and nowhere else (D41 rules .claude/session/ out explicitly: it is gitignored, so a colleague opening the committed dashboard would see nothing, and it holds current gate state rather than a sequence of events):',
        '  - runs/intents.log, the trivial and task intents: tab-separated, columns timestamp, tier, type, promptId, sessionId, summary, summary always last. Split each line on tabs and branch on the field count, never on the content: six or more fields is the post-D41 format, field five is the session id and the summary is field six onward rejoined with tabs, so a summary containing a tab is not corrupted; exactly five fields is a pre-D41 line, its session id is null and its summary is field five. The recorder now requires and validates --session before it writes anything, so every line it appends from here on has six fields and an empty fifth field cannot occur through the normal path. Known limitation of the legacy format, stated rather than papered over: a pre-D41 line whose own summary contained a tab also splits into six or more fields and is indistinguishable from a post-D41 line, so its first summary chunk will be read as a session id. Accept that reading; do not guess around it with heuristics on the value shape. kind is "intent", path is null.',
        '  - runs/intents/*.md, the substantive intent concepts: at from recorded_at, tier from tier, type from request_type, promptId from prompt_id, sessionId from session_id, summary from the body, path relative to the persistence root, kind is "intent".',
        '  - the run records under runs/ whose frontmatter type is "run": kind is "run", at from the frontmatter generated timestamp when present and the file mtime otherwise, summary from the title, path relative to the persistence root, tier and type null.',
        '  Selection, in exactly this order, so two runs over the same repository produce the same key:',
        '  1. collect every candidate event from the three sources;',
        '  2. sort newest first by at, breaking ties by source order, which for the log means file order;',
        '  3. keep the events inside the last 30 days, measured from the snapshot generatedAt;',
        '  4. floor: if that leaves fewer than 50, extend to the 50 newest overall, however old;',
        '  5. ceiling: cut to at most 500;',
        '  6. byte budget: while the JSON encoding of events exceeds 262144 bytes, drop from the oldest end, always at an event boundary and never mid-event;',
        '  7. omitted.count is how many candidates did not make it, omitted.oldestKept is the at of the oldest event kept (null when there are none), and omitted.reason names which limit bit last, one of "days", "ceiling", "bytes", or null when nothing was omitted.',
        '  A repository with no intents and no run records yields events: [] with omitted.count 0 and omitted.oldestKept null; report that emptiness as it is, so the view renders an honest empty state, and never invent an event to fill it.',
