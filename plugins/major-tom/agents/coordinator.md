...
1. Delegated work runs in the task's worktree, never directly on the feature branch: you decide when it is ready to go to the review phase, and it reaches the feature branch only after the verdict. Git belongs to the `vcs-operator` that owns it, and delegating it is the working model rather than a workaround for a tool you lack: ask that operator for every branch, worktree and merge operation, never run git yourself.
