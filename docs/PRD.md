...
- `plugins/major-tom`: 11 agents (the full roster of section 5, plus `onboard-check` for
  ...
  command, D31),
  `plugin.json` v0.17.0 (D30), `config.schema.json` at the plugin root (D3; JSON Schema
  ...
  one exempted `Bash` pattern, the OQ-15 deadlock fix), `skills/think/` (D36 as amended by
...
- `.claude-plugin/marketplace.json` v0.17.0 (D30), dual-plugin.
