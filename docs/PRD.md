...
(the generated copy of the dashboard server, refreshed on every onboard, D32). All three are
regenerated or recreated on demand and none of them is a project artifact. D43 adds the
generated dashboard artifact to that category and moves it out of the persistence root into
`.claude/server/`, beside the server that will serve it, on the same reasoning: a file that
carries no knowledge and only works with the mechanism running is mechanism state, so
`.knowledge/` goes back to holding only knowledge, which was D2's intent. That move is
decided and not yet implemented: today the artifact still sits inside the persistence root.
...
snapshot on demand, the artifact moving out of the persistence root to `.claude/server/`
beside the server, and the data island, `file://` viewing and `render.js inject` going with
it. The rest of this section describes what exists today.
...
the horizon, D42 for the floor). The session identity the view groups events by is persisted
