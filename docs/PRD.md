...
regenerated or recreated on demand and none of them is a project artifact. The two things
under version control stay under version control: `.claude/major-tom.json` is tracked because
...
the island. Snapshot schema v2 (D33, extended by D41) has eight keys: the six required ones
...
states until the track phase and the roadmap tool exist. Those producers, further design and
tech iteration, and the live-data endpoint stay in OQ-13.
...
- `plugins/major-tom`: 11 agents (the full roster of section 5, plus `onboard-check` for
  ...
  command, D31),
  `plugin.json` v0.16.0 (D30), `config.schema.json` at the plugin root (D3; JSON Schema
...
- `.claude-plugin/marketplace.json` v0.16.0 (D30), dual-plugin.
