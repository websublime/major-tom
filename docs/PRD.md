...
the mustache grammar, `render.js inject` replaces only the island. Snapshot schema v2 (D33)
has seven keys: the five required ones (`generatedAt`, `config`, `git`, `knowledge`,
`decisions`), which the onboard's snapshot builder produces today, and the two optional ones
the dashboard reads but no mechanism produces (`lastRun`, `roadmap`), which the builder omits
entirely so those views render their honest empty states by design rather than by accident.
...
Decided and not yet built (D41): a new snapshot key `timeline`, built from `.knowledge/runs/`
only (the `intents.log` lines, the intent concepts, the run records), and a sixth hash route
`#timeline` rendering it as its own view, with the overview carrying only the last five events
and a link to it. Neither the key nor the route exists today: nothing in
`templates/dashboard/src/` mentions the timeline and the onboard's snapshot builder does not
produce the key. The same entry decides that the git window moves from the 50 most recent
commits to the timeline's horizon, still unbuilt, and that the knowledge walk keeps only OKF
concepts, which is built (0.13.19): the snapshot builder now collects only non-reserved `.md`
files carrying frontmatter, so neither `.knowledge/runs/intents.log` nor the generated
`dashboard.html` reaches `knowledge.files`.
Window and retention for the timeline are settled by D41; producers for `lastRun` and
`roadmap`, plus further design and tech iteration, stay in OQ-13.
...
- `templates/` at the repository root (D16, D19), the authoring source shared by both
  ...
  `dashboard.html` (D34, D33), `dashboard-server.js` (D32), `launch-merge.js` (D32),
...
| OQ-13 | Narrowed by D31, D32, D33 and D41 (data island, inject mode, on-demand server, Desktop integration, vanilla-permanent, design ported, knowledge window 32 KB/file and 1 MB total). Settled by D41, the windowing and retention half: the runs window is hybrid, last 30 days with a floor of 50 events, a ceiling of 500 events and a 256 KB byte budget cut at an event boundary, per-event summaries capped at 200 characters with an explicit cut flag, and every omission stated in plain text in the view (`omitted: {count, oldestKept, reason}`). Retention: nothing is ever pruned at the source, `.knowledge/runs/` stays the source of truth, and older events stay reachable by opening the run files and `intents.log` directly, never by paginating a static HTML page; monthly log rotation (`intents-2026-08.log`) is the future move if the log grows unwieldy, deletion never is. Remaining: producers for the `lastRun` and `roadmap` snapshot keys (depend on OQ-3/OQ-4 and the track phase), further design iteration, and the live-data endpoint as the server's evolution. |
