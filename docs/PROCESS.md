...
- Design gate, mechanical: check 11 in `.github/checks.py` fails a change that edits `skills/` or `agents/` unless the same change carries a file under `docs/specs/` declaring `verdict`, `attacked_by` and `author`. Check 12 fails a commit touching `skills/`, `agents/` or `docs/specs/` whose message carries no `GOAL:` and `UNSETTLED:` line; a commit is bound by it only if its own tree already contains the check, so everything before it is exempt without a hardcoded date. Both need the base branch, which is why `.github/workflows/checks.yml` sets `fetch-depth: 0`; with no base they skip and say why rather than passing quietly.
