...
Across twelve runs, two variants, and two tiers, the drift trap fired exactly once: a bottom-tier control on the sparse variant, implementing the drifted plan verbatim. With the skill, 5 of 5 runs resolved the drift doc-first and shipped the right contract: on this family, ground-control's measurable value lives at the bottom tier under sparse evidence, consistent with the program's oldest pattern (the method is a floor-guard, rounds 1-5). The process layer is where everything else fails, always in the same shape: work landing on main without the human gate. Each round converted its failure into a consequence: the explicit merge gate (round 11), the TRACK line (round 12; on its first test it turned a hidden violation into a declared one), the local-only clarification (round 13), and the placement lesson (round 14: a forced artifact must live in the document the executor re-reads, the binding, not only the skill). The behavior gap stays open and honest: prose in the skill does not make Haiku branch; the confession field makes the violation impossible to hide, and the binding is the next, and last cheap, lever.
