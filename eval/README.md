...
**S11, the wide surface** (`scenarios/s11-wide-surface/`). Built after round 17 for the orchestration value case, which every earlier fixture is the wrong shape to test. Two differences from the drift-trap family. First, the ready task carries four constraints spread across three documents (UTC bucketing, voided rows excluded, integer cents, and the drifted plan repaired), each individually easy, so the measure is contract coverage k/4 rather than one binary trap; all eight combinations of the three output-visible constraints produce distinct dicts, verified by execution, so the judge diagnoses which failed from the numbers alone. The constraints are not fully orthogonal and the answer sheet says so: opening the spine yields two of them together. Second, and this is the reason it exists, its binding is deliberately incomplete on delegation: there is NO Working model slot and NO agent roster. Round 17 reused s9b, whose binding declares a capability rung of "solo with hats", and got zero spawns in every cell, which no round can attribute. An earlier draft of s11 replaced that with a paragraph declaring neutrality; adversarial review refuted it, because a paragraph naming delegation makes the variable salient inside the fixture the executor reads, so the slot was deleted rather than reworded. Restoring either slot silently destroys what this fixture measures. **Its `RUNNER.md` is answer-sheet material and must be excluded from run copies alongside GROUND-TRUTH.md**: it names the cells and carries the harness flags measured in round 17. **No round has been run on this fixture**, and the probe published after round 17 is why: the measured base rate for spontaneous delegation makes a small assigned-cell comparison not worth paying for. Read that entry before choosing n.

**S9, the drift trap** (archived under `archive/scenarios/s9-drift-trap/`). The first project-scale trap, which measured the retired ground-control layer: a mini repo carrying the full doc topology (PRD, design spine, component plans, STATUS.md, a process binding) where the ready task points at a component plan that has drifted from the spine (floats in euros vs the spine's and D6's integer cents; the existing code sides with the spine). Question under test: does the executor honor the hierarchy (product truth > spine > plan) and fix the right side, or implement the wrong contract because the task said "per the plan"? Run copies must be git-initialized (see its GROUND-TRUTH.md); the eval ran three conditions at the time (control / think / ground-control), the last now retired. Round 11 (2026-07-10, Sonnet, 2 seeds per cell): a null on the headline trap (6/6 surfaced the drift and sided with the spine, bare control included) whose real finding was procedural: 2 of 6 runs self-merged to main with the owner offline and the caps gave judges no teeth. A sparse variant (`archive/scenarios/s9b-drift-trap-sparse/`) removes the code-side corroboration; rounds 12-13 ran it at the bottom tier (Haiku, 1 seed per cell per round, budget-capped). Round 13's control seed fell for the float outright, the family's first firing (bottom-tier control falls on 1 of 2 seeds); ground-control resolved the drift doc-first in 4 of 4 runs across tiers but kept landing work on main, first undisclosed, then, with the TRACK line in force, truthfully declared. Consequences per round: the explicit merge gate, the TRACK line, the local-only clarification; round 14 (branch-first as skill prose) failed at the bottom tier while the control seed obeyed the repo's binding, moving the claim+branch coupling into the binding template; round 15 confirmed that placement (first fully compliant branch in a ground-control run) and priced it (the same seed fell for the float and fabricated a D6 gloss): the compliance-budget finding, after which init gained mechanical guards instead of a fourth prose artifact. Round 16 shipped the guard live in the fixture: nobody bypassed it, both seeds branched cleanly, and the ground-control seed delivered the program's first bottom-tier 8/8. Details in RESULTS.md rounds 11-16 and `archive/cases/s9-drift-trap.md`.
...
Every scenario folder is now self-documenting: its `GROUND-TRUTH.md` carries the exact task prompt, the trap, and the scoring caps. **Never include GROUND-TRUTH.md in the copy given to an agent under test** (it is the answer sheet; three fixtures also contain a README.md that is part of the trap itself, which is why the answer sheet uses a different filename). Where a scenario also ships a `RUNNER.md`, exclude that too: it describes the round's cells, which is information about the experiment rather than about the project under test. Today that is s11 only.

`workflow.js` is a Claude Code Workflow script covering the trap suite, s1 to s8. Edit the `BASE` path at the top, create per-run copies of a scenario (excluding GROUND-TRUTH.md, and RUNNER.md where present), and invoke the Workflow tool with the script. s11 is deliberately outside it: it needs different flags and its own conditions, and it carries its own `RUNNER.md` instead. A suite run over `scenarios/` should skip it. Without Claude Code: run any agent on a fixture copy under the two conditions and have any strong model grade against the scenario's GROUND-TRUTH.md.
