...
**S11, the wide surface** (`scenarios/s11-wide-surface/`). Built after round 17 for the orchestration value case, which every earlier fixture is the wrong shape to test. Two differences from the drift-trap family. First, the ready task carries FOUR independent constraints spread across three documents (UTC bucketing, voided rows excluded, integer cents, and the drifted plan repaired), each individually easy, so the measure is contract coverage k/4 rather than one binary trap; every wrong combination produces a distinct output dict, so the judge diagnoses which constraints failed from the numbers alone. Second, and this is the reason it exists, its binding is deliberately NEUTRAL on delegation: the Working model slot says the shape of the work is not prescribed and there is no agent roster. Round 17 reused s9b, whose binding declares a capability rung of "solo with hats", and got zero spawns in every cell, which no round can attribute. Restoring the working-model slot or the roster in this fixture silently destroys what it measures; its GROUND-TRUTH.md says so, and `RUNNER.md` carries the measured harness flags.
