...
Viewing has two surfaces (D31, amended by D32). CLI: `/major-tom:dashboard` starts the
localhost server and opens the browser. Claude Desktop: onboard merges a managed
...
| OQ-13 | Narrowed by D31 and D32 (data island, inject mode, v1 views, on-demand server, Desktop launch.json integration, vanilla-permanent all decided). Remaining: snapshot window and retention numbers (git window is 50 commits for now, runs windowing undecided), how older runs stay reachable, design iteration inside the vanilla file, and the live-data endpoint as the server's evolution. |
